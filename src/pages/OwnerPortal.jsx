...
import { searchOwnerItemsPublic } from '../services/publicCheckService';
...
export default function OwnerPortal() {
  ...
  const [ownerWithdrawalsList, setOwnerWithdrawalsList] = useState([]);
  ...
  // Fungsi Pencarian Pemilik via Cloud Function Server (Aman & Tidak membuka Firestore ke publik)
  const handleSearch = async (e) => {
    if (e) e.preventDefault();
    const query = searchInput.trim();
    if (!query || query.length < 2) {
      setErrorMessage('Silakan masukkan Nama atau No. Handphone (minimal 2 karakter)');
      return;
    }
    ...
    try {
      const response = await searchOwnerItemsPublic(query);

      if (response && response.found) {
        setSelectedOwner(response.owner);
        setOwnerItems(response.items || []);
        setOwnerWithdrawalsList(response.withdrawals || []);
      } else {
        setSelectedOwner(null);
        setOwnerItems([]);
        setOwnerWithdrawalsList([]);
        setErrorMessage(
          response?.message ||
            `Data penitip dengan kata kunci "${query}" tidak ditemukan. Pastikan nama atau nomor HP sesuai dengan yang terdaftar di Fitbay.id.`
        );
      }
    } catch (err) {
      console.error('Error during owner search:', err);
      setSelectedOwner(null);
      setErrorMessage(err.message || 'Terjadi gangguan saat mencari data. Silakan coba kembali.');
    } finally {
      setSearching(false);
    }
  };
  ...
  const waWithdrawalUrl = useMemo(() => {
    ...
    const message = `Halo Admin Fitbay.id, saya *${selectedOwner.name}* ingin mengecek status dan mengajukan pencairan dana barang titipan saya.\n\nMohon informasi lebih lanjut dari Admin.\nTerima kasih!`;
    return `https://wa.me/${ADMIN_WA_NUMBER}?text=${encodeURIComponent(message)}`;
  }, [selectedOwner, ADMIN_WA_NUMBER]);
      ...
      <main className="max-w-6xl mx-auto px-4 lg:px-8 py-6 lg:py-10">
        ...
        <section className="mb-8">
          <div className="max-w-2xl mx-auto text-center mb-6">
            <h2 className="text-2xl lg:text-3xl font-extrabold tracking-tight mb-2">
              Pantau Status Barang Titip Jual Anda
            </h2>
            ...
            <form onSubmit={handleSearch} className="flex flex-col sm:flex-row items-center gap-2">
              <div className="relative w-full">
                ...
                <input
                  ...
                  disabled={searching}
                  className="w-full pl-10 pr-4 py-3 rounded-2xl dark:bg-surface-200 bg-white border dark:border-white/10 border-gray-300 text-sm focus:outline-none focus:border-accent focus:ring-2 focus:ring-accent/20 transition-all shadow-sm disabled:opacity-60"
                />
              </div>
              <button
                type="submit"
        ...
        {selectedOwner && (
          <div className="space-y-7 animate-fade-in">
            ...
            <div className="p-6 rounded-3xl dark:bg-surface-200 bg-white border dark:border-white/10 border-gray-200 shadow-sm flex flex-col md:flex-row items-start md:items-center justify-between gap-5">
              <div className="flex items-center gap-4">
                <div className="w-13 h-13 rounded-2xl bg-gradient-to-br from-accent to-purple flex items-center justify-center text-white font-extrabold text-xl shadow-md uppercase">
                  {selectedOwner.name.charAt(0)}
                </div>
                <div>
                  <div className="flex items-center gap-2.5">
                    <h3 className="text-xl font-bold dark:text-white text-gray-900 capitalize tracking-tight">
                      {selectedOwner.name}
                    </h3>
                    <span className="text-[11px] font-semibold px-2.5 py-0.5 rounded-full bg-accent/15 text-accent border border-accent/20">
                      Penitip Terverifikasi
                    </span>
                  </div>
                  <p className="text-xs dark:text-gray-400 text-gray-500 mt-1">
                    No. Handphone: <span className="font-mono font-medium">{selectedOwner.phoneMasked || '-'}</span>
                  </p>
                </div>
              </div>
              ...
              <a
                ...
                className="w-full md:w-auto px-6 py-3 rounded-2xl bg-[#25D366] hover:bg-[#1EBE5D] text-white font-bold text-xs transition-all shadow-md hover:shadow-lg hover:shadow-[#25D366]/20 flex items-center justify-center gap-2.5"
              >
                ...
                <span>Hubungi Admin Fitbay</span>
              </a>
            </div>

            {/* Kartu Metrik Ringkasan Barang */}
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div className="p-5 rounded-2xl dark:bg-surface-200 bg-white border dark:border-white/10 border-gray-200 shadow-sm flex flex-col justify-between">
                ...
                <p className="text-2xl lg:text-3xl font-extrabold dark:text-white text-gray-900 tracking-tight">
                  {stats.totalItems} <span className="text-sm font-normal dark:text-gray-400 text-gray-500">item</span>
                </p>
              </div>

              <div className="p-5 rounded-2xl dark:bg-surface-200 bg-white border border-amber-500/20 shadow-sm flex flex-col justify-between">
                <span className="text-xs font-bold text-amber-400 uppercase tracking-wider block mb-2">
                  Stok Ready (Belum Terjual)
                </span>
                <p className="text-2xl lg:text-3xl font-extrabold text-amber-400 tracking-tight">
                  {stats.readyItems} <span className="text-sm font-normal text-amber-400/70">item</span>
                </p>
              </div>

              <div className="p-5 rounded-2xl dark:bg-surface-200 bg-white border border-emerald-500/20 shadow-sm flex flex-col justify-between">
                <span className="text-xs font-bold text-emerald-400 uppercase tracking-wider block mb-2">
                  Barang Terjual
                </span>
                <p className="text-2xl lg:text-3xl font-extrabold text-emerald-400 tracking-tight">
                  {stats.soldItems} <span className="text-sm font-normal text-emerald-400/70">item</span>
                </p>
              </div>
            </div>
                    ...
                    {filteredItems.map((item) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="pt-2.5 border-t dark:border-white/5 border-gray-100 flex items-center justify-between text-[10px] dark:text-gray-500 text-gray-400">
                            <span>Tgl Titip: {item.tanggalMasuk ? formatDate(item.tanggalMasuk) : '-'}</span>
                            {isSold && item.tanggalTerjual && (
                              <span className="text-emerald-400 font-medium">Laku: {formatDate(item.tanggalTerjual)}</span>
                            )}
                          </div>
                        </div>
                      );
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] dark:text-gray-400 text-gray-500">
                              Tanggal: {w.date ? formatDate(w.date) : '-'}
                            </p>
                            {w.notes && (
